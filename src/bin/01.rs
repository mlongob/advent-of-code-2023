...
fn convert_spelled_out_digits(line: &str) -> String {
    lazy_static! {
        static ref RE: Regex = Regex::new("(one|two|three|four|five|six|seven|eight|nine)").unwrap();
    }
...
fn convert_spelled_out_digits_reversed(line: &str) -> String {
    lazy_static! {
        static ref RE: Regex = Regex::new(")one|two|three|four|five|six|seven|eight|nine(".chars().rev().collect::<String>().as_str()).unwrap();
    }
    RE.replace_all(line.chars().rev().collect::<String>().as_str(), |cap : &Captures| {
        match cap[0].chars().rev().collect::<String>().as_str() {
            "one" => "1",
            "two" => "2",
            "three" => "3",
            "four" => "4",
...
pub fn part_two(input: &str) -> Option<u32> {
    Some(input.lines().filter_map(|l| {
        let first = convert_spelled_out_digits(l).chars().find(char::is_ascii_digit)?.to_digit(10)?;
        let last = convert_spelled_out_digits_reversed(l).chars().rfind(char::is_ascii_digit)?.to_digit(10)?;
        Some(first * 10 + last)
    }).sum())
}
